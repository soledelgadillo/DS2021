search_for_filenames_containing_any_of_the_patterns_and_output_file_info <-
        ...
        # We could also "run the loop" in this fashion:
        # for (S in search.strings) {
        #       V <- list.files(directory, pattern = S, full.names =FALSE,
        #                ignore.case = TRUE) 
        # and the rest of the for loop
        if (num_files == 0) {
        ...
        num_files <- length(filenames) 
        # need to do this since we eliminated the duplicate names
        ...
        # scrlisting.txt (chose a different directory)
        
        output_filename <- paste(
                "/Users/pipu/Documents/DS Specialization/DS2021/2 R Programming/Practice/Programming Exercises/Exercise 7/Output",
                "/", "scrlisting.txt", sep = "")
